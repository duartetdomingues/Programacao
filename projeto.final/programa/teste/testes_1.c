...
void write_file(char *, country_data *, char *);
...
int main(int argc, char *argv[])
{
    ...
    printf("antes da escrita no ficheiro");
    write_file(out_filename, list_head, cabecalho);
    return 0;
}
...
country_data *read_file(char *in_filename, char *leitura, char *selecao, char *restricao, int *restricao_populacao, char *restricao_data, char *restricao_data_2, country_data *list_head)
{
    ...
    weekly_count = (int *)malloc(10*sizeof(int));
    cumulative_count = (int *)malloc(10*sizeof(int));
    int_population = (int *)malloc(20*sizeof(int));

    rate_14_day = (double *)malloc(10*sizeof(double));
    ...
    else if (strstr(in_filename, ".dat") != NULL)
    {
        ...
        iteration = fread(&num_country, sizeof(int), 1, fp_in); //ler o numero de estruturas de paises
        //printf("num_country= %d\n",num_country);
            for (i = 0; i != num_country; i++)
            {
                iteration =fread(&num_cases, sizeof(int), 1, fp_in);                       //ler o numero de estruturas de cases
                iteration =fread(&num_deaths, sizeof(int), 1, fp_in);                      //ler o numero de estruturas de deaths

                iteration = fread(&len_country, sizeof(int), 1, fp_in);
                
                //free(str_country);
                str_country = (char *)malloc(len_country);
                iteration =fread(&(*str_country), len_country, 1, fp_in);

                printf("%s,", str_country);
                iteration =fread(&len_country_code, sizeof(int), 1, fp_in);

                free(str_country_code);
                str_country_code = (char *)malloc(len_country_code);             //malloc fdd
                iteration =fread(&(*str_country_code), len_country_code, 1, fp_in);  
                

                printf("%s,", str_country_code);
                iteration =fread(&len_continent, sizeof(int), 1, fp_in);
                free(str_continent);         
                str_continent = (char *)malloc(len_continent);
                iteration =fread(&(*str_continent), len_continent, 1, fp_in);
                
                printf("%s,", str_continent);
                //printf("address %d\n", &population_temp);

                free(int_population);
                int_population = (int *)malloc(sizeof(int));
                iteration =fread(&(*int_population), sizeof(int), 1, fp_in); //seg fault
                ...
                for(j=0; j != (num_cases+num_deaths)  ; j++)
                {
                    ...
                    iteration =fread(&len_indicator, sizeof(int), 1, fp_in);
                    indicator = (char *)malloc(len_indicator);
                    iteration =fread(&(*indicator), len_indicator, 1, fp_in);
                    printf("%s,",indicator);

                    weekly_count = (int *)malloc(sizeof(int));
                    iteration =fread(&(*weekly_count), sizeof(int), 1, fp_in);
                    printf("\n%d,", *weekly_count);

                    iteration =fread(&len_year_week, sizeof(int), 1, fp_in); 
                    str_year_week = (char *)malloc(len_year_week);
                    iteration =fread(&(*str_year_week), len_year_week, 1, fp_in);
                    printf("%s,", str_year_week);
                    
                    rate_14_day = (double *)malloc(sizeof(double));
                    iteration =fread(&(*rate_14_day), sizeof(double), 1, fp_in);
                    printf("%lf,", *rate_14_day);
                    
                    cumulative_count = (int *)malloc(sizeof(int));
                    iteration =fread(&(*cumulative_count), sizeof(int), 1, fp_in);
                    printf("%d", *cumulative_count);
                    ...
                    //printf("%d\n",list_head);
                    //printf("vou print");
                    //write_file("testes.csv", list_head, cabecalho);
...
void write_file(char *out_filename, country_data *list_head, char *cabecalho)
{
    int len_country=0, len_country_code=0, len_continent=0, len_year_week=0,len_indicator=0 ,int_population=0, int_weekly_count=0, int_cumulative_count=0, population_temp=0;
    ...
    if (strstr(out_filename, ".dat") != NULL)
    {
        ...
        fp_out = fopen(out_filename, "wb");
            ...
            for (Aux = list_head; Aux != NULL; Aux = Aux->nextC)
            {
                ...
                (population_temp = *Aux->population);
                //len_population = strlen(Aux->country)+1;
                //printf("len_country= %d\n", len_country);
                //printf("len_country_code= %d\n", len_country_code);
                //printf("len_len_continent= %d\n", len_continent);
                ...
                //printf("%d\n", *Aux->population);
                //printf("population yeah %d\n", population_temp);
                fwrite(&population_temp, sizeof(int), 1, fp_out);
                //printf("%d\n", population_temp);
