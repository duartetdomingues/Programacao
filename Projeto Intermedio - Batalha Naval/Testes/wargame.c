...
{ // Função que lê as opções do programa e o direciona para o modo de Jogo correspondente
    ...
    { // ler as opções escritas pelo utilizador na consola
        ...
        case 't':
            ...
            { // verificar se o tabuleriro inserido é valido
                dim_tabu_x = DEFAULT_DIM_TABU_X;
                dim_tabu_y = DEFAULT_DIM_TABU_Y;
            }
            break;
        case 'j':
        ...
        case 'd':
            ...
            if (modo_jogo != 2)
            { // verificar se no modo de jogo pretendido pode ser definido modo de disparo // ATENÇÃO: NOTAMOS QUE NO SCRIPT QUE NOS FORNECEU ESTA RESTRIÇÃO PROVOCA UM CONFLITO, CONTUDO NO ENUNCIADO É REFERIDA ESTA RESTRIÇÃO.
    ...
    if (((dim_tabu_y * dim_tabu_x) / 9) < (peca_n[1] + peca_n[2] + peca_n[3] + peca_n[4] + peca_n[5] + peca_n[6] + peca_n[7] + peca_n[8]))
    { // Verificar Restrição 2
        printf("\n-1 : O número de peças excedeu o número maximo de peças da matriz\n\n");
        return (1);
    }

    switch (modo_jogo)
    { // Encaminhar o programa para o modo de jogo pretendido
    case 0:
        modo_j0();
        break;
    case 1:
...
{ // Modo de Jogo 0
    ...
    switch (modo_pos)
    { // Encaminhar o programa para o modo de posicionamento pretendido
    case 1:
        modo_p1();
        break;
    case 2:
...
{ // Modo de Jogo 1
    ...
    switch (modo_pos)
    { // Encaminhar o programa para o modo de disparo pretendido
    case 1:
        modo_p1();
        break;
    case 2:
    ...
    { // verificar se o jogador ainda não descobriu todas as peças ou se houve algum erro
        erro = 0;
        scanf("%s", resposta);
        ...
        else
        {
            ...
            if (x_alfa < 65 || (x_alfa > (dim_tabu_x + 64) && x_alfa < 96) || x_alfa > (dim_tabu_x + 96) || y <= 0 || y > dim_tabu_y)
            { // verificar se as coordenasdas introduzidas são invalidas
                printf("\n-1 : Coordenada inválida! Introduza novamente.\n\n");
                erro = 1;
            }
        }
        if (x_alfa <= dim_tabu_x + 64)
        { // verificar se inseriu letras Maiúsculas
            x = x_alfa - 64;
        }
        else
        {
...
{ // Print do Tabuleiro
    int l, i, j, k;
    l = dim_tabu_y;
    for (i = dim_tabu_x + 1; i <= 25; i++)
    { // Apaga todas as coordenadas da matriz "yx" para além da dimensão do tabuleiro de jogo
        for (j = 0; j <= 16; j++)
        {
            yx[j][i] = ' ';
        }
    ...
    {
        if (l < 10)
        { // Se o número das linhas for < 10 escreve um espaço antes do número para as matrizes ficarem alinhadas
            printf(" ");
        }
    ...
    printf("  ");
    for (k = 65; k <= (dim_tabu_x + 64); k++)
    { // escreve as letras correspondentes as colunas do tabuleiro
        printf(" %c", k);
    }
    printf("\n");
}
...
void print_logo()
{ // Print do Cabeçalho do Programa
    if (modo_jogo != 0)
    { // Se o modo de jogo for diferente de 0, pois ele não escreve esta parte do cabeçalho no modo de jogo 0
        printf("* ================================\n");
        switch (modo_jogo)
        { // Escreve o cabeçalho consoante o modo de jogo
        case 1:
            printf("* Modo de Jogo 1\n");
