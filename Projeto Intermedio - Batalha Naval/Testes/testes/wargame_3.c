...
time_t t;
    ...
    int main( int argc, char *argv[] ){
        ...
        while ((option= getopt(argc, argv, "ht:j:p:d:1:2:3:4:5:6:7:8:")) != -1){
            switch(option){
                ...
                case 'd':sscanf(optarg, "%d", &modo_dis);/*if(modo_jogo!=2){printf("\n-1 : No modo de jogo escolhido não é defenido o modo de disparo\n");instrucoes();}*/break;
                case '1':sscanf(optarg, "%d", &peca_n[1]);if(modo_pos==1 && (modo_jogo==0 || modo_jogo==1)){printf("\n-1 : No modo de jogo escolhido não são definidas as Peças\n");instrucoes();}break;
                case '2':sscanf(optarg, "%d", &peca_n[2]);if(modo_pos==1 && (modo_jogo==0 || modo_jogo==1)){printf("\n-1 : No modo de jogo escolhido não são definidas as Peças\n");instrucoes();}break;
                case '3':sscanf(optarg, "%d", &peca_n[3]);if(modo_pos==1 && (modo_jogo==0 || modo_jogo==1)){printf("\n-1 : No modo de jogo escolhido não são definidas as Peças\n");instrucoes();}break;
                case '4':sscanf(optarg, "%d", &peca_n[4]);if(modo_pos==1 && (modo_jogo==0 || modo_jogo==1)){printf("\n-1 : No modo de jogo escolhido não são definidas as Peças\n");instrucoes();}break;
                case '5':sscanf(optarg, "%d", &peca_n[5]);if(modo_pos==1 && (modo_jogo==0 || modo_jogo==1)){printf("\n-1 : No modo de jogo escolhido não são definidas as Peças\n");instrucoes();}break;
                case '6':sscanf(optarg, "%d", &peca_n[6]);if(modo_pos==1 && (modo_jogo==0 || modo_jogo==1)){printf("\n-1 : No modo de jogo escolhido não são definidas as Peças\n");instrucoes();}break;
                case '7':sscanf(optarg, "%d", &peca_n[7]);if(modo_pos==1 && (modo_jogo==0 || modo_jogo==1)){printf("\n-1 : No modo de jogo escolhido não são definidas as Peças\n");instrucoes();}break;
                case '8':sscanf(optarg, "%d", &peca_n[8]);if(modo_pos==1 && (modo_jogo==0 || modo_jogo==1)){printf("\n-1 : No modo de jogo escolhido não são definidas as Peças\n");instrucoes();}break;
            }
        }

        for (k=2;k<=8;k++){
            if(peca_n[k]>peca_n[k-1]){printf("\n-1 : Nunca pode haver mais peças de um tipo de maior dimenção do que de um tipo de menor dimenção\n");instrucoes();}
        }
        if (((dim_tabu_y*dim_tabu_x)/9)<(peca_n[1]+peca_n[2]+peca_n[3]+peca_n[4]+peca_n[5]+peca_n[6]+peca_n[7]+peca_n[8])){printf("\n-1 : O numero de peças exedeu o numero maximo de peças da matriz\n");instrucoes();}
    ...
    void modo_p2(){
        ...
        if (n_pecas_escolhidas>n_pecas_max){printf("\n-1 : O numero de peças exedeu o numero maximo de peças da matriz\n");instrucoes();}
