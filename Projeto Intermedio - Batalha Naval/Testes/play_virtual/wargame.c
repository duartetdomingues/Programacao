...
time_t t; 
        ...
        while ((option= getopt(argc, argv, "ht:j:p:d:1:2:3:4:5:6:7:8:")) != -1){ // ler as opções escritas pelo utilizador na consola
            switch(option){
                ...
                case 'd':
                    ...
                    /*if(modo_jogo != 2){ // verificar se no modo de jogo pretendido pode ser definido modo de disparo
                        printf("\n-1 : No modo de jogo escolhido não é definido o modo de disparo.\n\n");
                        instrucoes();
                    }
                    break;*///DEPOIS DO SCRIPT DO STOR POR ESTA OPÇÃO- so não funciona no script do stor porque o stor adiciona ao que metemos -j 1 -p 2 e nos metemos a opção -d (1,2 ou 3) mas na realidade não podemos fazer isso.
                case '1':
    ...
    void modo_j1(){ // Modo de Jogo 1
        ...
        char  x_alfa, resposta[256], d_yx[17][26];
    ...
    void instrucoes(){ // Mensagem de Ajuda
        ...
        printf(" O modo de jogo 0 só será válido se o invocar da seguinte forma:\n"); 
    ...
    void modo_p2(){ // Modo de Posicionamento 2
        int n_pecas_total, n_pecas_max, matriz_x, matriz_y, x, y, selecao_tipo, variante, tentativa_v, tentativa_p[9]={0}, n_peca[9]={0}, i, j, i_tabuleiro=0, erro_tabuleiro=0, erro_peca=0, e_v=0;
